    ...
    try: 
        ...
        for i in range(num_steps):
            ...
            for j in range(jsteps):
                ...
                print('loss1:',loss1)
                print('loss2:',loss2)
                adap=0.3*float(loss1.data/(1/loss2.data))
                # print(adap)
                loss1=loss1+adap*(1/loss2)
                loss1.backward()
                input_grad=input1.grad.data
                # input_grad = torch.sign(input_grad)
                #inverse_rescale
            ...
            if loss < min_loss:
                print("### lesser than min_loss", loss, " < ", min_loss)
                print("loss: ", loss)
                min_loss = loss
                save_img_i(patch_fix,i, output_dir + "adv_stop/")
                det_and_save_img_i(input1, i, output_dir + "adv_img")
