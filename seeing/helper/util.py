...
from helper.bbox import bbox_iou
...
def write_results(prediction, confidence, num_classes, nms = True, nms_conf = 0.4):
    ...
    for ind in range(batch_size):
        ...
        image_pred = torch.cat(seq, 1)#到这里是完成了转化过程，[batch_size,10647=3*(13*13+26*26+52*52),5+80]---》[batch_size,10647,5+2]
                                      #10647=anchor_num*(feature_size1^2+f_size2^2+f_size3^2); 5=xc,yc,width/2,height/2,confidence;2=class_num,confidence_class

        
        #Get rid of the zero entries
        non_zero_ind =  (torch.nonzero(image_pred[:,4]))#根据anchor box的confidence列去掉0值保留非0值
        
        image_pred_ = image_pred[non_zero_ind.squeeze(),:].view(-1,7)
        
        #Get the various classes detected in the image
        try:
            img_classes = unique(image_pred_[:,-1]) #去除留下来的类别列中重复的类别
        except:
             continue
