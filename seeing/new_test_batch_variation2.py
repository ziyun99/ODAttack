...
def get_feature_dist(start_x,end_x,start_y,end_y,feature_out,feature_target):
       # dist={}
        dist=torch.zeros(len(feature_out)).cuda()
...
#DISTANCE WITHOUT SOFTMAX
            num_index=(end_fx-start_fx)*(end_fy-start_fy)
            dist_one=torch.sum((f_st-f_sa)**2)/num_index
          #  print('dist without softmax:',dist_one)            


# DISTANCE WITH SOFTMAX : the values are too small, the distance with softmax is not so suitable          
            #mean
          #  f_samean = torch.sum(f_sa,2)
          #  f_samean = torch.sum(f_samean,1)/num_index
          #  f_stmean = torch.sum(f_st,2)
          #  f_stmean = torch.sum(f_stmean,1)/num_index
            #generalization
          #  f_stmean=torch.softmax(f_stmean,0)
          #  f_samean=torch.softmax(f_samean,0)
          #  dist_one=torch.sum((f_stmean-f_samean)**2)


          #  dist=dist+dist_one
            dist[i]=dist[i]+dist_one
      #      print('dist[i]:',dist[i])
...
def save_img2(img,i):
    ...
    cv2.imwrite('new_record/yue_record_'+str(i)+'.png', img_save)
#    cv2.imwrite('test_multi_layer/mat_data/input.png', img_save)

def save_img3(img,i):
    img_save = inp_to_image(img)
    cv2.imwrite('new_input_record/yue_record_ori_'+str(i)+'.png', img_save)
 #   cv2.imwrite('test_multi_layer/mat_data/input_target.png', img_save)    
...
def get_gradients(patch_fix, original_stop,img_ori,map_4_patches,ori_pole,ni):
        start_x = 200
        start_y = 100
        width = 201
        height = 201
        ...
        if i%3==0:
            patch_transform[:,:,:],ori_stop2[:,:,:]=addweight_transform_multiple(patch_fix[:,:,:],original_stop[:,:,:])#gamma_transform_multiple(patch_fix[:,:,:])
...
# gradients for prediction layer loss
        loss.backward(retain_graph=True)
        input_grad=input1.grad.data
# SAVE LOSS_DISAPPEAR GRAD
   #     save_fn = 'test_multi_layer/mat_data/grad_loss.mat'
   #     scio.savemat(save_fn, {'y':input_grad.cpu().data.numpy()})
        
   #     print('loss.backward().torchsum:',torch.sum(input_grad))
       # loss_test=2*loss2
       # loss_test.backward(retain_graph=True)
      #  input_grad=input1.grad.data
      #  print('loss_test:',input_grad)
        ...
        grad_loss1=grad_resize

       # input_grad = torch.sign(input_grad)
#inverse_rescale
# gradients for hidden layers loss
        ratio_layers={}
        mean_layer={}
        for j in range(len(loss2)):
              if (j+1)<len(loss2):
                  ...
                  input_grad=input1.grad.data
        #          print('loss2.backward().torchsum'+str(j)+':',torch.sum(input_grad)) 
        #          save_fn = 'test_multi_layer/mat_data/grad'+str(j)+'.mat'
        #          scio.savemat(save_fn, {'y':input_grad.cpu().data.numpy()})           
              else:
                  loss2[j].backward()
                  input_grad=input1.grad.data
        #          print('loss2.backward().torchsum'+str(j)+':',torch.sum(input_grad))
                 ...
                 # save_fn = 'test_multi_layer/mat_data/grad'+str(j)+'.mat'
                 # scio.savemat(save_fn, {'y':input1.grad.data.numpy()}) 
              input_grad=input1.grad.data
              grad_resize = input_grad[0,:,start_x:end_x,start_y:end_y]
              grad_resize=cv2.resize(grad_resize.cpu().numpy().transpose(1,2,0),(width,height),cv2.INTER_CUBIC)
              if(perspective==1):
              ...
              mean_layer[j]=np.sum(abs(grad_resize))
         #     print('mean_layer:',mean_layer)
              if j==0:
                 grad_layer=grad_resize
                 ratio_layer=1
              else:
                 ratio_layer=mean_layer[0]/mean_layer[j]#set the first layer'mean value as the baseline
                # print('ratio:',ratio_layer)
                 grad_layer=grad_layer+grad_resize*ratio_layer
              ratio_layers[j]=ratio_layer
        #gradients for both prediction layer and hidden layers
        mean_loss1=np.sum(abs(grad_loss1))
        mean_loss2=np.sum(abs(grad_layer))
        grad_all=grad_loss1#+0.5*(mean_loss1/mean_loss2)*grad_layer
        grad_all=torch.from_numpy(grad_all.transpose(2,0,1)).cuda()
        return grad_all,loss,loss2,ratio_layers
...
#
        ...
        for j in range(jsteps):
            grad_resize1,loss1,loss2,ratio_layers=get_gradients(patch_fix, original_stop,img_ori,map_4_patches,ori_pole,i*jsteps+j)
            if j==0:
        ...
        print('loss:',loss)
        print('loss2:',loss2)
        print('ratio_layers:',ratio_layers)
# add epsilon
...
#save_ori.png
 ...
 #   output=get_adv_opt(img_ori,adv_label,first_index,ori_index)
    ...
    prediction,feature_out = model(output_adv , CUDA)
